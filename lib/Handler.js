...
async function updateBookId(request, response){
const bookId = request.params.id;

try{
    const updatedBook = await Book.findByIdAndUpdate(bookId, request.body, {new: true});
    if(!updatedBook){
        return response.status(404).json({message: 'Book not found'});
    }
    response.status(200).json(updatedBook);
}catch(error){
    console.error('Error has occurred while updating book', error);
    response.status(500).send('Error occurred while updating book' , error);
}


}

module.exports = {newBook, getBooks, getBooksID, deleteBooksID, updateBookId};